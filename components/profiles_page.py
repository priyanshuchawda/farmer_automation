...
def render_profiles_page():
    ...
    st.header("👥 Manage Farmer Profiles")
    ...
    <div style='background-color:#E3F2FD;padding:15px;border-radius:10px;margin-bottom:20px;border-left:4px solid #1976D2;'>
        ...
        <small>💡 New farmers register from the login page. Use this page to view all farmer profiles and manually add new ones if needed.</small>
    </div>
        ...
        if submitted:
            if name and location and farm_size > 0 and contact and password and confirm_password:
                if password != confirm_password:
                    ...
                    st.success(f"✅ Farmer '{name}' profile saved successfully! You can now login with your credentials.")
            else:
