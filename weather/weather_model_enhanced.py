...
def should_retrain_model(data_path=None, model_path=None):
    """Check if the model needs to be retrained."""
    weather_dir = os.path.dirname(os.path.abspath(__file__))
    if data_path is None:
        data_path = os.path.join(weather_dir, 'pune_weather_cleaned.csv')
    if model_path is None:
        model_path = os.path.join(weather_dir, 'weather_model_enhanced.pkl')
...
def train_and_save_model():
    ...
    # STEP 1 — LOAD DATA
    # -------------------------
    df = pd.read_csv(os.path.join(weather_dir, 'pune_weather_cleaned.csv'))
    ...
    weather_dir = os.path.dirname(os.path.abspath(__file__))
    joblib.dump(model_data, os.path.join(weather_dir, 'weather_model_enhanced.pkl'))
...
def forecast_next_7_days(target_date=None):
    ...
    if should_retrain_model():
        ...
        model_data = joblib.load(os.path.join(weather_dir, 'weather_model_enhanced.pkl'))
